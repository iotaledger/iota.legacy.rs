...
impl<'a> Transaction for TransactionView<'a> {
    ...
    fn extra_data_digest(&self) -> Hash {
        Hash::from_trits(&self.0[EXTRA_DATA_OFFSET..ADDRESS_OFFSET]).unwrap()
    }
    ...
    fn bundle_nonce(&self) -> Nonce {
        Nonce::from_trits(&self.0[BUNDLE_NONCE_OFFSET..TRUNK_OFFSET]).unwrap()
    }
    ...
    fn branch(&self) -> Hash {
        Hash::from_trits(&self.0[BRANCH_OFFSET..TAG_OFFSET]).unwrap()
    }

    fn tag(&self) -> Tag {
        Tag::from_trits(&self.0[TAG_OFFSET..ATTACHED_AT_OFFSET]).unwrap()
    }
